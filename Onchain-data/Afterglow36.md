...
六、心得体会与知识点总结   
External 函数： hello() 使用 external 关键字，意味着它只能被外部账户或其它合约调用，不能被合约内部函数调用（而 public 都可以）。     
Events 的作用： Events（事件）是智能合约与链下应用（如前端 DApp、分析工具）通信的主要方式。它们不存储在合约状态中，但永久地存储在交易日志中，可以被高效地索引和查询。    
